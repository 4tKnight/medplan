...
class Weight extends StatefulWidget {
  String dependentId ;
  Weight({super.key, this.dependentId=''});
...
class _WeightState extends State<Weight> {
  ...
  String content =
      "Strictly speaking, body weight is the measurement of mass without items located on the person. Practically though, body weight may be measured with clothes on, but without shoes or heavy accessories such as mobile phones and wallets, and using manual or digital weighing scales. A person's weight can affect their health in many ways, including increasing the risk of developing serious diseases";
  ...
  fetchRecord() async {
    ...
    try {
      var res = await _healthRecordServices.viewWeight(
      ...
      if (res['status'] == 'ok') {
        if (res['health_record'] != null) {
          ...
          minReading = data['fitness_weight']['min_reading'] ?? 0;
          maxReading = data['fitness_weight']['max_reading'] ?? 0;
          avgReading = (data['fitness_weight']['avg_reading'] ?? 0).toInt();
          readings = data['fitness_weight']['readings'] ?? [];
        } else {
          minReading = 0;
          maxReading = 0;
          avgReading = 0;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: helperWidget.myAppBar(context, 'Weight'),
      body: ListView(
    ...
    return InkWell(
      onTap: () {
        myWidgets.buildAddBottomSheet(
          ...
          year: year,
          apiCall: (recordData) {
            return _healthRecordServices.addWeight(
              dependentId: widget.dependentId,
              reading: recordData,

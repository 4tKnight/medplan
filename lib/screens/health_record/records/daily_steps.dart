...
class DailySteps extends StatefulWidget {
  String dependentId ;
   DailySteps({super.key,this.dependentId = ''});
...
class _DailyStepsState extends State<DailySteps> {
  ...
  String fieldName = 'daily_steps_value';
  ...
  fetchRecord() async {
    ...
    try {
      var res = await _healthRecordServices.viewDailySteps(
      ...
      if (res['status'] == 'ok') {
        if (res['health_record'] != null) {
          ...
          minReading = data['fitness_daily_steps']['min_reading'] ?? 0;
          maxReading = data['fitness_daily_steps']['max_reading'] ?? 0;
          avgReading =
              (data['fitness_daily_steps']['avg_reading'] ?? 0).toInt();
          readings = data['fitness_daily_steps']['readings'] ?? [];
        } else {
          minReading = 0;
          maxReading = 0;
          avgReading = 0;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: helperWidget.myAppBar(context, 'Daily Steps'),
      body: ListView(
    ...
    return InkWell(
      onTap: () {
        myWidgets.buildAddBottomSheet(
          context: context,
          title: 'Enter Daily Steps',
          recordName: 'Daily Steps',
          fieldName: fieldName,
          month: month,
          year: year,
          apiCall: (recordData) {
            return _healthRecordServices.addDailySteps(
              dependentId: widget.dependentId,
              reading: recordData,
